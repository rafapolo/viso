...
// Worker code is embedded as string blobs; only the factory functions need TS types.

export const createDataProcessingWorker = (): Worker => {
  const workerCode = `
    let duckdb = null;
    let db = null;
    let conn = null;
...
export const createBackgroundSyncWorker = (): Worker => {
  const workerCode = `
    let taskQueue = [];
      ...
      try {
        switch (task.type) {
          case 'download':
            ...
            self.postMessage({ type: 'taskComplete', taskId: task.id, result: { data, size: data.byteLength } });
            break;
          case 'cleanup':
            await new Promise(resolve => setTimeout(resolve, 100));
            self.postMessage({ type: 'taskComplete', taskId: task.id, result: { cleaned: 5 } });
            break;
          case 'sync':
            await new Promise(resolve => setTimeout(resolve, 200));
            self.postMessage({ type: 'taskComplete', taskId: task.id, result: { synced: true } });
            break;
          default:
        ...
        self.postMessage({ type: 'taskError', taskId: task.id, error: error.message });
    ...
    const processQueue = async () => {
      ...
      while (taskQueue.length > 0) {
        ...
        if (task.status === 'pending') await processTask(task);
      }
      isProcessing = false;
    };
    ...
    self.onmessage = async (event) => {
      ...
      try {
        switch (type) {
          case 'addTask':
            const task = { id: data.id || Date.now().toString(), type: data.type, status: 'pending', retryCount: data.retryCount || 3, ...data };
            taskQueue.push(task);
            processQueue();
            self.postMessage({ type: 'taskAdded', taskId: task.id });
            break;
          case 'getQueueStatus':
            self.postMessage({ type: 'queueStatus', status: {
          ...
          case 'clearCompleted':
            ...
            self.postMessage({ type: 'tasksCleared', count: beforeCount - taskQueue.length });
            break;
          case 'startSync':
    ...
    self.addEventListener('online', () => { self.postMessage({ type: 'onlineStatusChanged', online: true }); processQueue(); });
    self.addEventListener('offline', () => { self.postMessage({ type: 'onlineStatusChanged', online: false }); });
  `;
